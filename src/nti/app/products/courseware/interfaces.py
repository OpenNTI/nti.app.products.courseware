...
class IEnrollmentEvent(interface.Interface):
	user = interface.Attribute("User in enrolling event")
	context = interface.Attribute("Context object")
	
class IUserCourseEnrollEvent(IEnrollmentEvent):
	"""
	Fired when a user is enrolled in course 
	"""
	course = interface.Attribute("Course/context object")
	
class IUserCourseDropEvent(IEnrollmentEvent):
	"""
	Fired when a user drops a course 
	"""
	course = interface.Attribute("Course/context object")

@interface.implementer(IEnrollmentEvent)
class EnrollmentEvent(ObjectEvent):
...
@interface.implementer(IUserCourseEnrollEvent)
class UserCourseEnrollEvent(EnrollmentEvent):
	
	@property
	def course(self):
		return self.context

@interface.implementer(IUserCourseDropEvent)
class UserCourseDropEvent(EnrollmentEvent):
