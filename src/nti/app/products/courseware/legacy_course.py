...
def _register_course_purchasable_from_catalog_entry(entry, event):
    ...
    # Some things were different with the courses produced for Fall 2013
    old_rendering = False
    ...
    # some other IComponents is defined (which should be the non-persistent
    # baseregistery.BaseComponents)
...
def _update_scopes(course, purchsable_ntiid, package):  # pylint:disable=I0011,W0212
    ...
    if ES_PUBLIC not in scopes:
        scopes._SampleContainer__data[ES_PUBLIC] = find_interface(
            course, ICommunity)

    if ES_CREDIT not in scopes:
        legacy_scopes = get_scopes_for_purchasable_ntiid(purchsable_ntiid)
        if legacy_scopes is None:
            children = package._v_toc_node.iterchildren(tag='course')
            course_element = next(children)
...
def _course_instance_for_catalog_entry(entry):
    ...
    provider = get_provider(entry.ContentPackageNTIID)
    policy = component.queryUtility(
        ICourseCatalogLegacyEntryInstancePolicy, name=provider)
    if policy is None:
        return None
