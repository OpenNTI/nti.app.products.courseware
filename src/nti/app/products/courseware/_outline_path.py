...
class OutlinePathFactory(object):
    ...
    def _get_slidedeck_for_video(self, target_ntiid, lesson_overview):
        ...
        for slide_deck in self.catalog.search_objects(container_ntiids=content_ntiid,
    ...
    def _overview_item_contains_target(self, item, check_contained=True):
        ...
        # Some related work refs have hrefs pointing at content (instead of
        # target).
        ...
        if      not result \
            ...
            if target_ref_obj is not None:
                ...
                if not result:
                    ...
                    try:
                        target_children = [
                            x.ntiid for x in target_ref_obj.children
                        ]
                ...
                if lesson_overview is not None:
                    if lesson_ntiid == self.target_ntiid:
                        results = (self.course_context,
                                   outline_content_node, self.target_obj)
                    else:
