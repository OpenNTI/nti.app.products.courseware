...
def _get_self_assessments_for_course(course):
    ...
    return [x for x in result if x.ntiid not in qsids_to_strip]
...
def _do_get_containers_in_course(context):
    ...
    containers_in_course = containers_in_course.union(
        self_assessment_containerids)
    ...
    assignment_ntiids = (
        asg.ntiid for asg in assignment_catalog.iter_assignments()
    )
...
def _get_containers_in_course(context):
    ...
    key = "/course/%s" % encode_keys(site, entry.ntiid,
                                     "containers", last_synchronized())
