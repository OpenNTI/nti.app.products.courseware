...
class TestCourseRecursiveTransactionHistory(ApplicationLayerTest):
	...
	catalog_ntiid = 'tag:nextthought.com,2011-10:NTI-CourseInfo-Fall2013_CLC3403_LawAndJustice'
	...
	def _get_txs(self, _):
		...
		if not self.tx_pool:
			...
			for i in range(TX_POOL_SIZE):
				record = TransactionRecord(principal='martin_chatwin',
										   tid='%s' % i,
								  	 	   attributes=tuple())
				txs.append(record)
			shuffle(txs)
	...
	def test_audit_log(self, mock_get_txs):
		...
		log_link = self.require_link_href_with_rel(res.json_body, VIEW_RECURSIVE_AUDIT_LOG)
		...
		self.next_tx_idx = 0
		res = self.testapp.get(log_link + '?sortOrder=%s&batchSize=%s' % ('descending', 50))
		res = res.json_body
		records = res.get(ITEMS)
