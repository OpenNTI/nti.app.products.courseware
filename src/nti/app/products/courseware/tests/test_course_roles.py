...
class TestCourseRoles(ApplicationLayerTest):
    ...
    def test_roles(self):
        ...
        # Editor
        self.create_user('three-fifty-five')
        three_environ = self._make_extra_environ('three-fifty-five')
        # Neither
        self.create_user('hero.brown')
        ...
        remove_editor_href = self.require_link_href_with_rel(
                                            course,
                                            VIEW_COURSE_REMOVE_EDITORS)
        remove_instructor_href = self.require_link_href_with_rel(
                                            course,
        ...
        assert_that(editors, contains_inanyorder('jmadden', 'harp4162', 'three-fifty-five'))
        ...
        # Remove editor
        self.testapp.post_json(remove_editor_href, {'user': 'three-fifty-five'})
        self.testapp.post_json(remove_editor_href, {'user': 'three-fifty-five'})
        editors = _editor_names()
        assert_that(instructors, has_length(2))
        assert_that(instructors, contains_inanyorder('jmadden', 'harp4162'))
